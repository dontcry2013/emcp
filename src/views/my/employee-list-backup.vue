	        ...
	        this.getMessage({"from": "0", "to": step.toString()}, function(mData){
	        	if(mData){
	        		...
	        		var temp = mData.filter(function(itm, pos){
	        			var ret =  true;
	        			for (var i = 0; i < app.vueApp.$store.state.appData.employees.length; i++) {
        					if(app.vueApp.$store.state.appData.employees[i].id == itm.id){
        						return false;
        					}
        				}
        				return ret;
	        		...
	        		if(temp.length > 0){
	        			currentPage += step;
	        			mData = temp.concat(app.vueApp.$store.state.appData.employees);
		...
		tabUp:function(self){
	        ...
	        this.getMessage({"from": (currentPage+1).toString(), "to": (currentPage+step+1).toString()}, function(mData){
	        	if(mData){
	        		...
	        		if(mData.length > 0){
	        			currentPage += step;
	        			mData = app.vueApp.$store.state.appData.employees.concat(mData);
		        		that.items = mData;
		        		app.vueApp.$store.dispatch("updateEmployees", mData);
	        		}
	        	}
